import { calculateMonthlyTotal, salaryReplacementPercentage, disciplineConsistency, quitJobReadiness } from "../src/lib/calculations"; // keep extensionless for ts-node resolution
...
const entries = [{ amount: 100 }, { amount: 200 }, { amount: 0 } as any];
assert(calculateMonthlyTotal(entries as any) === 300, "monthly total should be 300");
...
assert(disciplineConsistency(d) === 67 || disciplineConsistency(d) === 66, "consistency approx 66/67%");
